...
function getTaoItem(url){
	...
	$.getJSON(ajax_url,function(result){
		...
		if(data.laiyuan=='天猫'||data.laiyuan=='淘宝'){
			if(data.taoke!=1){
				alert('商品没参加淘客');
			}
		}
...
<table id="addeditable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
  ...
  <tr>
    ...
    <td>&nbsp;<input type="text" id="url" name="url" value="<?=$row['url']?>" style="width:300px" /> <input onClick="getTaoItem($('#url').val())" class="sub" id="getGoodsItem" type="button" value="获取商品信息" /><span class="zhushi"><?=$goods_tishi['r']?></span></td>
  </tr>
   <tr>
...
<tr>
  ...
  <tr>
    ...
    <td>&nbsp;<input name="img" type="text" id="img" value="<?=$row['img']?>" style="width:300px"  /><?php if($row['img']!=''){?><img style="width:25px; height:25px; margin-bottom:-8px; *margin-bottom:-6px;" src="<?=tb_img($row['img'],50)?>" /><?php }?>  <input onClick="javascript:openpic('<?=u('fun','upload',array('uploadtext'=>'img','sid'=>session_id()))?>','upload','450','350')" class="sub" type="button" value="上传" /> <span class="zhushi">可使用远程图片地址 <a href="<?=SITEURL.'/'.$row['img']?>" target="_blank">查看</a></span></td>
  </tr>
