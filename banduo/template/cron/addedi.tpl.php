...
<table id="addeditable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
  ...
  <tr>
    ...
    <td>&nbsp;<?=html_radio($fangshi_arr,$row['fangshi'],'fangshi')?> <label class="execute_time">每天的<select name="execute_time" id="xiaoshi"><?php for($i=0;$i<24;$i++){?><option <?php if($i==$row['execute_time']){?> selected="selected"<?php }?> value="<?=$i?>"><?=$i?>时</option><?php }?></select></label> <!--<span class="execute_time">分钟</span><select id="fenzhong" class="execute_time"><?php for($i=0;$i<60;$i++){?><option value="<?=$i?>"><?=$i?>分</option><?php }?></select>-->  <label class="interval_time"><input type="text" class="w5" value="<?=$row['interval_time']?>" id="interval_time" name="interval_time"><span class="zhushi">单位分钟，如果需要间歇1小时，填写60，表示每隔多长时间执行一次任务</span></label> <label class="random"><input type="text" id="random" value="<?=$row['random']?>" name="random" class="w5"><span class="zhushi">基值是1000,1是千分之一的概率，以此类推</span></label></td>
  </tr>
  <!--<tr>
