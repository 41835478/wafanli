...
class goods{
	...
	function show($pagesize,$page=1,$field='*',$where="1=1",$order_by){
		...
		$a=$this->duoduo->select_all($this->table_name,$field,$where.$order_by.' limit '.$frmnum.','.$pagesize);
	...
	function do_item($item,$goods_attribute_arr=array(),$goods_type=array()){
		...
		if($item['tg_url']){
			$item['jump']=SITEURL.'/index.php?mod=jump&act=s8&url='.base64_encode($item['tg_url']).'&iid='.iid_encode($item['data_id']);
