...
function dd_act(){
	...
	elseif($q!=''){
		$url=$goods['jump']="index.php?mod=jump&act=s8&url=&name=".urlencode($q);
		jump($url);
	}
	...
	$tao_goods['jump']=u('jump','goods',array('iid'=>iid_encode($tao_goods['iid'])));
	$tao_goods['shop_jump']=u('jump','shop',array('user_id'=>$tao_goods['user_id'],'nick'=>$tao_goods["nick"]));
	...
	if($is_url && !(TAO_SEARCH_URL || $dduser['search']==1)){
		...
		if(WEBTYPE==0){
			jump(u('jump','goods',array('iid'=>iid_encode($iid))));
		}
