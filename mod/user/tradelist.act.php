...
function act_user_tradelist($pagesize = 10) {
	...
	if ($do == 'taobao') {
		...
		$dingdan = $duoduo->select_all('tradelist', '*', "uid=" . $dduser['id'] . " and (checked=2 or checked=3) and del=0 order by ".(TAOTYPE==1?'create_time':'pay_time')." desc,id desc limit $frmnum,$pagesize");
